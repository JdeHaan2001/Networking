...
namespace server
{
	...
	class LoginRoom : SimpleRoom
	{
		...
		private void handlePlayerJoinRequest(PlayerJoinRequest pMessage, TcpMessageChannel pSender)
		{
			...
			{
				Console.WriteLine($"NAME IS UNIQUE, playerInfo count: {_server.GetPlayerInfo((playerInfo) => playerInfo.name == pMessage.name).Count}");
				PlayerInfo newPlayerInfo = new PlayerInfo();
				newPlayerInfo.name = pMessage.name;

				Log.LogInfo($"Moving new client {newPlayerInfo.name} to accepted...", this);
				...
				Console.WriteLine("Player info added");
				return;
			}
			else if (_server.GetPlayerInfo((playerInfo) => playerInfo.name == pMessage.name).Count > 0)
			{
