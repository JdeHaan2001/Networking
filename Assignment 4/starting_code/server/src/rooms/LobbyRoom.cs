...
namespace server
{
	...
	class LobbyRoom : SimpleRoom
	{
		...
		protected override void addMember(TcpMessageChannel pMember)
		{
			...
			GetClientName clientName = new shared.GetClientName();
			clientName.ClientName = playerInfo.name;
			pMember.SendMessage(clientName);
			...
			simpleMessage.message = $"Client {playerInfo.name} has joined the lobby!";
		...
		protected override void handleNetworkMessage(ASerializable pMessage, TcpMessageChannel pSender)
		{
			...
			else if (pMessage is ChatMessage) handleChatMessages(pMessage as ChatMessage, pSender);
		}
		...
		private void handleChatMessages(ChatMessage pMessage, TcpMessageChannel pSender)
		{
			...
			outMessage.message = $"[{playerInfo.name}] {pMessage.message}";
